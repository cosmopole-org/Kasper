...
import { Point } from "../sigma/models";
...
let hookStateStore = {
	...
	overlayData: hookstate<{ workerId: string, code: string, room: Point } | null>(null),
	overlayLoaded: hookstate(false),
	loadingOverlay: hookstate(false),
	currentAppletData: hookstate<{ id: string, room: Point, code: string } | undefined>(undefined),
	minimizedApplets: hookstate<{ [id: string]: { room: Point, code: string } }>({}),
...
export let Actions = {
	...
	switchAppletShown: (v: boolean, workerId?: string, room?: Point) => {
		if (v && workerId && room) {
	...
	updateOverlayData: (v: { workerId: string, code: string, room: Point } | null) => {
		States.store.overlayData.set(v);
	},
	switchOverlayLoaded: (v: boolean) => {
		States.store.overlayLoaded.set(v);
	},
	updatePos: (pointId: string) => {
		States.store.currentPos.set({ pointId });
	},
	...
	openCreateTopicModal: (pointId: string) => {
		RouteSys.push("/app/create-topic", { pointId });
	},
