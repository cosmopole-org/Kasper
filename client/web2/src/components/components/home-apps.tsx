...
export default function HomeApps() {
    ...
    useEffect(() => {
        const membersObservable = api.store?.db.collections.members.find({
            selector: {
                pointId: {
                    $eq: pos.pointId
                }
    ...
    const chat = useMemo(() => <Chat pointId={pos.pointId} />, [pos.pointId]);
    const files = useMemo(() => <Files />, []);
    const bots = useMemo(() => <MachinesList className='pt-2' machines={members} />, [members]);
