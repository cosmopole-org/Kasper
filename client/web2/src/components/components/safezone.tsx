...
const Safezone = (props: { stateKey?: string, isWidget?: boolean, code: string, machineId?: string, identifier?: string, room?: Point, onCancel: () => void, overlay?: boolean }) => {
    let id = props.room ? `${props.machineId}::${props.identifier}` : props.machineId
    if (!id) id = ''
    const [show, setShow] = useState(false)
    const [ready, setReady] = useState(false)
    const preparedIframeData = useRef('')
    const identifier = props.room ? `safezone-${props.machineId + "::" + props.identifier}` : `safezone-${props.machineId + "::0"}`
    ...
    useEffect(() => {
        const messageCallback = (e: any) => {
            ...
            if (workerId && (workerId === id)) {
                if (data.key === 'onLoad') {
                    ...
                    if (props.room) {
                        // api.services?.points.ask({ recvId: workerId.startsWith('desktop-sheet-') ? workerId.substring('desktop-sheet-'.length) : workerId, data: data.packet, spaceId: props.room.parentId, topicId: props.room.id });
                    }
        ...
        let packetReceiver = api.services?.addListener('points/signal', (packet: any) => {
            ...
            if (data.type === "response") {
                if (props.identifier) {
                    let wi = props.identifier.startsWith('desktop-sheet-') ? props.identifier.substring('desktop-sheet-'.length) : props.identifier;
        ...
        return () => {
            api.services?.delListener('ponts/signal', packetReceiver ?? "");
