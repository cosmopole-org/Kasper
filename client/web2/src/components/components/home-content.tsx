...
export default function HomeContent() {
    ...
    const [topic, setTopic] = useState<Point | null>(null);
    useEffect(() => {
        api.store?.db.collections.points.findOne({ selector: { id: { $eq: pos.pointId } } }).exec().then((t: any) => {
            setTopic(t);
        });
    }, [pos.pointId]);
    const board = useMemo(() => (topic ? <Desk point={topic} show={true} /> : null), [wallpaper, theme, showBoardBg, topic?.id]);
    const members = useMemo(() => <HomeAvatarGroup pointId={pos.pointId} />, [pos.pointId]);
