...
let openAppletSheet = (_room: Point, _workerId: string) => { }
...
const AppletSheet = () => {
    ...
    React.useEffect(() => {
        ...
        let getAppletEvent = api.services?.addListener('points/signal', (packet: any) => {
        ...
        return () => {
            api.services?.delListener('points/signal', getAppletEvent ?? "");
            window.removeEventListener('message', messageCallback)
        }
    }, [])
    closeAppletSheet = () => Actions.switchAppletShown(false)
    openAppletSheet = (room: Point, workerId: string) => {
        workerIdRef.current = workerId
        roomRef.current = room
        Actions.switchAppletShown(true, workerId, room)
        api.services?.points.signal(room.id, workerId, 'single', JSON.stringify({ tag: 'get/applet', theme: appTheme, secondaryColor: colors.purple, colorName: "blue", colors: colors.blue }));
    }
