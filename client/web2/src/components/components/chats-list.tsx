...
import { User, Point } from "@/api/sigma/models";
...
export default function ChatsList({ currentCat }: { currentCat: string }) {
    ...
    useEffect(() => {
        ...
        let spacesSub = spacesObservable?.subscribe(ss => {
            let dict: { [id: string]: Point } = {};
            ss.forEach(s => { dict[s.id] = s as any; })
            setSpacesDict(dict);
        });
        const topicsObservable = api.store?.db.points.find().$;
        let topicsSub = topicsObservable?.subscribe(ts => {
            setTopics(ts.map(t => t as any));
        });
        return () => {
    ...
    return (
        <div
            ...
            {
                topics.filter((item) => (
                    (currentCat === "$.0") ? ((interactsDict[item.parentId] != undefined) && (interactsDict[item.parentId] as any).participant.type === "human") :
                        (currentCat === "$.1") ? (interactsDict[item.parentId] === undefined) :
                            ((interactsDict[item.parentId] != undefined) && (interactsDict[item.parentId] as any).participant.type === "machine"))).map((item, index) => {
                                if (item.parentId == "" ? item.peerId : spacesDict[item.parentId].peerId) {
                                    let participant = (interactsDict[item.parentId] as any).participant;
                                    return (
                                        <Card onPress={() => RouteSys.push('/app/chat', { spaceId: item.parentId, topicId: item.id })} className="mt-4 m-h-16 w-full bg-transparent" key={item.id} isPressable shadow="none">
                                    ...
                                    return (
                                        <Card onPress={() => RouteSys.push('/app/chat', { pointId: item.id })} className="mt-4 m-h-16 w-full bg-transparent" key={item.id} isPressable shadow="none">
                                            <div className="flex gap-2 w-full">
                                                <Avatar isBordered alt={item.title} className="w-[56px] h-[56px] m-1" src={getUsers()[index].avatar} style={{ minWidth: 56 }} />
                                                <div className="flex flex-col relative w-full">
                                                    <div className="w-full flex flex-row mt-1">
                                                        <span className="text-lg text-left">
                                                            {item.title}
                                                        </span>
                                                        <Chip className={"ml-2 text-white"} size="sm" variant="faded" style={{ backgroundColor: colorMap[stringToColour((item.parentId == "" ? item.title : spacesDict[item.parentId].title) + " " + (item.parentId == "" ? item.avatar : spacesDict[item.parentId].avatar), colorList)][600] }}>
                                                            {item.parentId == "" ? item.title : spacesDict[item.parentId].title}
                                                        </Chip>
                                                    </div>
                                                    <span className="text-md text-default-400 text-left">No messsages exist in home...</span>
                                                    <span className="absolute right-0 top-1 text-sm">
