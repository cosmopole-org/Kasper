...
func main() {
	...
	for i := 0; i < len(args); i++ {
		...
		pluggerName := strings.Join(pluggerPathParts[1:], "/")
		code := `
		...
		for _, e := range entries {
			...
			code += `
			plugger_` + serviceName + ` "kasper/cmd/babble/sigma/` + pluggerName + `/pluggers/` + serviceName + `"
			action_` + serviceName + ` "kasper/cmd/babble/sigma/` + pluggerName + `/actions/` + serviceName + `"
			`
		}
		code += `
		)
...
func build(pluggerName string, serviceName string, serviceRoot string) {
	...
	import (
		...
		module_logger "kasper/cmd/babble/sigma/core/module/logger"
		actions "kasper/cmd/babble/sigma/` + pluggerName + `/actions/` + serviceName + `"
