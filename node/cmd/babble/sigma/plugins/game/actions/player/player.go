...
import (
	...
	game_inputs_player "kasper/cmd/babble/sigma/plugins/game/inputs/player"
	game_model "kasper/cmd/babble/sigma/plugins/game/model"

	"kasper/cmd/babble/sigma/abstract"
	game_outputs_player "kasper/cmd/babble/sigma/plugins/game/outputs/player"
	...
	social_model "kasper/cmd/babble/sigma/plugins/social/model"
	"strings"
...
var increaments = map[string]map[string]string{
	"hokm": {
		"video": "hokmgemad",
	},
}
...
func (a *Actions) Inc(s abstract.IState, input game_inputs_player.IncInput) (any, error) {
	...
	val := meta.Data[incKeyData].(string)
...
var jsStore = map[string]string{
	...
	"hokm->chatCharge": `
		function() {
			var now = Date.now();
			return Math.max(0, Math.ceil(((lastChatBuy + (chat * 24 * 60 * 60 * 1000)) - now) / (24 * 60 * 60 * 1000)));
		}
	`,
}
