...
import (
	...
	game_inputs_daily "kasper/cmd/babble/sigma/plugins/game/inputs/random"
	game_model "kasper/cmd/babble/sigma/plugins/game/model"
	game_outputs_daily "kasper/cmd/babble/sigma/plugins/game/outputs/random"
	"log"
	"math"
	"math/rand"
	"strconv"
...
func (a *Actions) Daily(s abstract.IState, input game_inputs_daily.DailyInput) (any, error) {
	...
	_, ok := userData["newlyCreated"]

	if ok && (userData["newlyCreated"].(bool)) {
		rndIndex = 4
	}
