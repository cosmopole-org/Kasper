...
func (wm *Wasm) WasmCallback(dataRaw string) string {
	...
	if key == "log" {
		...
		wm.sigmaCore.ExecAppletResponseOnChain(callbackId, []byte(pack), int(resCode), e, []abstract.Update{{Data: "kvstore: " + changes}}, []abstract.CacheUpdate{})
		...
		wm.sigmaCore.ExecAppletRequestOnChain(targetMachineId, k, []byte(pack), machineId, func(b []byte, i int, err error) {
			if err != nil {
				log.Println(err)
				return
			}
