...
// 	statel1 "kasper/cmd/babble/sigma/layer1/module/state"
...
// 					stateOfReq := h.wasmTool.sigmaCore.Get(1).Sb().NewState(moduleactormodel.NewInfo(machineId, spaceId, topicId, memberId)).(*statel1.StateL1)
// 					h.wasmTool.storage.DoTrx(func(trx adapters.ITrx) error {
// 						stateOfReq.SetTrx(trx)
// 						_, res, err := h.wasmTool.sigmaCore.Get(1).Actor().FetchAction("/topics/send").Act(stateOfReq, inp)
