...
import (
	...
	inputs_interact "kasper/cmd/babble/sigma/api/inputs/interact"
	inputs_spaces "kasper/cmd/babble/sigma/api/inputs/spaces"
	inputs_users "kasper/cmd/babble/sigma/api/inputs/users"
	model "kasper/cmd/babble/sigma/api/model"
	outputs_interact "kasper/cmd/babble/sigma/api/outputs/interact"
	outputs_spaces "kasper/cmd/babble/sigma/api/outputs/spaces"
	outputs_users "kasper/cmd/babble/sigma/api/outputs/users"
	moduleactormodel "kasper/cmd/babble/sigma/core/module/actor/model"
	"kasper/cmd/babble/sigma/layer1/adapters"
	modulestate "kasper/cmd/babble/sigma/layer1/module/state"
	moduletoolbox "kasper/cmd/babble/sigma/layer1/module/toolbox"
	"log"
	"sort"
	"strings"
...
func (a *Actions) Create(s abstract.IState, input inputs_interact.InteractDto) (any, error) {
	...
	if codeMap[-1] {
		interaction = model.Interaction{UserIds: a.getMergedUserIds(state.Info().UserId(), userId, input.Origin())}
...
func (a *Actions) ReadBlockedList(s abstract.IState, input inputs_interact.ReadBlockedListDto) (any, error) {
	...
	for _, user := range users {
		...
		err := trx.Db().Model(&model.User{}).Select(adapters.BuildJsonFetcher("metadata", "hokm.profile")).Where("id = ?", user.Id).First(&profileStr).Error
		if err != nil {
			log.Println(err)
			trx.ClearError()
			continue
