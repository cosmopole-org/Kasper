...
// AddMember /spaces/addUser check [ true true false ] access [ true false false false POST ]
func (a *Actions) AddUser(s abstract.IState, input inputsspaces.AddMemberInput) (any, error) {
	toolbox := abstract.UseToolbox[*tb.ToolboxL1](a.Layer.Tools())
	state := abstract.UseState[modulestate.IStateL1](s)
	var member models.Member
	trx := state.Trx()
	if trx.GetBytes("User",  != "" {
