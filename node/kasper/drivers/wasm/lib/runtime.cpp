...
void set_txn_db(TransactionDB *txdb)
{
    txn_db = txdb;
}
...
WasmThreadPool::WasmThreadPool(size_t num_threads)
{
...
void WasmThreadPool::enqueue(function<void()> task)
{
    {
...
int WasmUtils::parseDataAsInt(vector<char> buffer)
{
...
vector<char> WasmUtils::pickSubarray(vector<char> A, int i, int j)
{
...
bool WasmUtils::startswith(const std::string &str, const std::string &cmp)
{
    return str.compare(0, cmp.length(), cmp) == 0;
}

std::string WasmUtils::pickString(vector<char> A, int i, int j)
{
...
// ChainTrx Implementation -------------------------------------------------

ChainTrx::ChainTrx(std::string machineId,
                   std::string key,
                   std::string input,
                   std::string userId,
...
void WasmMac::registerHost(std::string modPath)
{
    ...
    WasmEdge_ValType Params0[6] = {WasmEdge_ValTypeGenI32(), WasmEdge_ValTypeGenI32(), WasmEdge_ValTypeGenI32(), WasmEdge_ValTypeGenI32(), WasmEdge_ValTypeGenI32(), WasmEdge_ValTypeGenI32()};
    WasmEdge_ValType Returns0[1] = {WasmEdge_ValTypeGenI64()};
    this->registerFunction(HostMod, "runDocker", runDocker, Params0, 6, Returns0);
    ...
    WasmEdge_ValType Params7[8] = {WasmEdge_ValTypeGenI32(), WasmEdge_ValTypeGenI32(), WasmEdge_ValTypeGenI32(), WasmEdge_ValTypeGenI32(), WasmEdge_ValTypeGenI32(), WasmEdge_ValTypeGenI32(), WasmEdge_ValTypeGenI32(), WasmEdge_ValTypeGenI32()};
    WasmEdge_ValType Returns7[1] = {WasmEdge_ValTypeGenI64()};
    this->registerFunction(HostMod, "submitOnchainTrx", submitOnchainTrx, Params7, 8, Returns7);
...
WasmEdge_Result runDocker(void *data, const WasmEdge_CallingFrameContext *, const WasmEdge_Value *In, WasmEdge_Value *Out)
{
    ...
    uint32_t keyOffset = WasmEdge_ValueGetI32(In[2]);
    int keyL = WasmEdge_ValueGetI32(In[3]);
    uint32_t cnOffset = WasmEdge_ValueGetI32(In[4]);
    int cnL = WasmEdge_ValueGetI32(In[5]);
    ...
    j2["containerName"] = containerName;
    j["input"] = j2;
    std::string packet = j.dump();
...
WasmEdge_Result execDocker(void *data, const WasmEdge_CallingFrameContext *, const WasmEdge_Value *In, WasmEdge_Value *Out)
{
    ...
    uint32_t keyOffset = WasmEdge_ValueGetI32(In[2]);
    int keyL = WasmEdge_ValueGetI32(In[3]);
    uint32_t coOffset = WasmEdge_ValueGetI32(In[4]);
    int coL = WasmEdge_ValueGetI32(In[5]);
