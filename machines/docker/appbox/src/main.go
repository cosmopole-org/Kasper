...
assume there are 6 classes of commands:
    ...
    8. function call: when program asks for calling a function which previously defined in the program so that it will move
    ...
    10. cast operation: when the program asks for casting a piece of data to a target type. for example: "cast @num1 var to float"
		(where the example @num1 might be an integer or anything else)

also the program may do prompt-based-code-generation: also the program may ask you some prompts in compile time in the middle of code,
...
assume there is a Val structure which should hold a data of types:
    ...
    - callback
    {
        ...
        "data": {
            "value": {
                "funcId": {{ the target previously defined function identifier Val to be linked to this callback }}
            }
        }
    }
    - identifier
...
structure like below and just output it only:
{
    ...
    "data": {
        "name": "{{ it is a raw string which is name of host api to be called. it can only be one of these values:
			...
			- "delayed", to run a function with delay. it gets an array Val as parameter, the first item in that array is an identifier pointing
			  to a function and the second item is a number i32 Val which is the delay time before running the function.
			- "periodic", to run a function repeatedly and periodically with an interval. it gets an array Val as parameter, the first item in that
			  array is an identifier pointing to a function and the second item is a number i32 Val which is the interval time between executions of
			  the function.
...
if the program asks for casting a Val or expression to a specific type then put them in a structure like below:
{
    "type": "cast",
    "data": {
        "value": {{ any type of Val or an expression which can be resolved into an Val such as a arithmetic type, one of Val types, or a host call, or an functionCall type or an indexer }},
        "targetType": "{{ string name of the target type to cast value to, it can be: "i16", "i32", "i64", "f32", "f64", "bool", "string" }}"
    }
}

note: prompt-based-code-generation:
...
} and be sure it is a valid json.`
...
func processPacket(callbackId int64, data []byte) {
	...
	if callbackId == 0 {
		...
		case "compileApp":
			...
			res, err := client.Models.GenerateContent(ctx, "gemini-2.5-flash", genai.Text(input["code"].(string)),
			...
			log.Println("test 3....")
			log.Println(res.Candidates[0].Content.Parts[0].Text)
			if len(res.Candidates) > 0 {
				response := res.Candidates[0].Content.Parts[0].Text
				response = strings.Split(response, "```json")[1]
				response = response[:len(response)-len("```")]
				signalPoint("single", pointId, userId, map[string]any{"type": "compileAppRes", "ast": response, "callbackId": input["callbackId"]})
			}
		}
	} else {
