...
func (vm *Vm) RunDocker(imageName string, containerName string, runAsync bool, requesterId string, requesterSignature string, lockId string, inputFiles map[string]string) *model.File {
	kp, kl := bytesToPointer([]byte(imageName))
	asyncMode := "false"
	if runAsync {
		asyncMode = "true"
	}
	cp, cl := bytesToPointer([]byte(containerName + "|" + asyncMode + "|" + requesterId + "|" + requesterSignature + "|" + lockId))
