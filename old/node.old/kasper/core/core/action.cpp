...
ActionOutput SecAction::runAsFed(std::function<void(std::function<void(StateTrx *)>)> stateModifier, ITools *tools, std::string userId, DataPack payload, std::string signature)
{
    json input = json::parse(std::string(payload.data, payload.len));
    auto meta = this->intel->extractMeta(input);
