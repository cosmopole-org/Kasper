...
func run(a int64) int64 {
	...
	if args == "offchain" {
		trx.Chain.SubmitRawTrx("79@global", "test", []byte("{}"))
	}
	...
	trx.Db.Jobs.CreateAndInsert(&job)

	job.Users.CreateAndInsert(&User{
		Id:   "1",
		Name: "keyhan",
		Age:  27,
	})
	...
	// job.Users.DeleteById("1")
	// trx.Db.Users.DeleteById("1")

	logger.Log("-----------------------------------------------")
	logger.Log("users :")
	for _, u := range trx.Db.Users.Read("all", "Age", "desc") {
		logger.Log(u.Name + " " + strconv.Itoa(u.Age))
	}
	logger.Log("-----------------------------------------------")
	logger.Log("jobs :")
	for _, j := range trx.Db.Jobs.Read("all", "Income", "desc") {
		logger.Log(j.Title + " " + strconv.Itoa(j.Income))
		for _, u2 := range j.Users.Read("all", "Age", "asc") {
			logger.Log(u2.Name + " " + strconv.Itoa(u2.Age))
		}
	}
	logger.Log("-----------------------------------------------")
