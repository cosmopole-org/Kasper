...
public:
  ...
  invokeWithNotifier(const Runtime::Instance::FunctionInstance *FuncInst,
                     Span<const ValVariant> Params,
                     Span<const ValType> ParamTypes, int vmId,
                     bool (*isPaused)(int inx), void (*notifyInstCount)(int inx, uint64_t ic),
                     void (*check)(int inx), bool conExec);
...
private:
  ...
  Expect<void>
  continueFunctionKasper(Runtime::StackManager &StackMgr,
                         const Runtime::Instance::FunctionInstance &Func,
                         int vmId, bool (*isPaused)(int inx),
                         void (*notifyInstCount)(int inx, uint64_t ic), void (*check)(int inx));

  Expect<void>
  runFunctionKasper(Runtime::StackManager &StackMgr,
                    const Runtime::Instance::FunctionInstance &Func,
                    Span<const ValVariant> Params, int vmId,
                    bool (*isPaused)(int inx), void (*notifyInstCount)(int inx, uint64_t ic),
                    void (*check)(int inx));
  /// Run Wasm function.
