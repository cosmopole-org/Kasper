...
public:
  ...
  Expect<std::vector<std::pair<ValVariant, ValType>>>
  executeWithNotifier(std::string_view Func, Span<const ValVariant> Params = {},
                      Span<const ValType> ParamTypes = {}, int vmId = 0,
                      bool (*isPaused)(int inx) = NULL,
                      void (*notifyInstCount)(int inx, uint64_t ic) = NULL,
                      void (*check)(int inx) = NULL) {
    std::shared_lock Lock(Mutex);
    return unsafeExecuteWithNotifier(Func, Params, ParamTypes, vmId, isPaused,
                                     notifyInstCount, check);
  }

  Expect<std::vector<std::pair<ValVariant, ValType>>> conExecuteWithNotifier(
      std::string_view Func, Span<const ValVariant> Params = {},
      Span<const ValType> ParamTypes = {}, int vmId = 0,
      bool (*isPaused)(int inx) = NULL, void (*notifyInstCount)(int inx, uint64_t ic) = NULL,
      void (*check)(int inx) = NULL) {
    std::shared_lock Lock(Mutex);
    return unsafeConExecuteWithNotifier(Func, Params, ParamTypes, vmId,
                                        isPaused, notifyInstCount, check);
  }
...
private:
  ...
  Expect<std::vector<std::pair<ValVariant, ValType>>>
  unsafeExecuteFuncWithNotifier(
      const Runtime::Instance::ModuleInstance *ModInst, std::string_view Func,
      Span<const ValVariant> Params, Span<const ValType> ParamTypes, int vmId,
      bool (*isPaused)(int inx), void (*notifyInstCount)(int inx, uint64_t ic), void (*check)(int inx),
      bool conExec);

  Expect<std::vector<std::pair<ValVariant, ValType>>>
  unsafeExecuteWithNotifier(std::string_view Func,
                                Span<const ValVariant> Params,
                                Span<const ValType> ParamTypes, int vmId,
                                bool (*isPaused)(int inx),
                                void (*notifyInstCount)(int inx, uint64_t ic),
                                void (*check)(int inx));

  Expect<std::vector<std::pair<ValVariant, ValType>>>
  unsafeConExecuteWithNotifier(std::string_view Func,
                                   Span<const ValVariant> Params,
                                   Span<const ValType> ParamTypes, int vmId,
                                   bool (*isPaused)(int inx),
                                   void (*notifyInstCount)(int inx, uint64_t ic),
