...
WASMEDGE_CAPI_EXPORT WasmEdge_Result WasmEdge_VMExecuteWithNotifier(
    WasmEdge_VMContext *Cxt, const WasmEdge_String FuncName,
    const WasmEdge_Value *Params, const uint32_t ParamLen,
    WasmEdge_Value *Returns, const uint32_t ReturnLen, int vmId,
    bool (*isPaused)(int inx), void (*notifyInstCount)(int inx, uint64_t ic), void (*check)(int inx));


WASMEDGE_CAPI_EXPORT WasmEdge_Result WasmEdge_VMConExecuteWithNotifier(
    WasmEdge_VMContext *Cxt, const WasmEdge_String FuncName,
    const WasmEdge_Value *Params, const uint32_t ParamLen,
    WasmEdge_Value *Returns, const uint32_t ReturnLen, int vmId,
    bool (*isPaused)(int inx), void (*notifyInstCount)(int inx, uint64_t ic), void (*check)(int inx));
...
///
/// After registering a WASM module in the VM context, you can repeatedly
/// call this function to invoke exported WASM functions by their module
/// names and function names until the VM context is reset. If the `Returns`
/// buffer length is smaller than the arity of the function, the overflowed
/// return values will be discarded.
///
/// This function is thread-safe.
...
/// \param Params the WasmEdge_Value buffer with the parameter values.
/// \param ParamLen the parameter buffer length.
/// \param [out] Returns the WasmEdge_Value buffer to fill the return
/// values.
/// \param ReturnLen the return buffer length.
///
/// \returns WasmEdge_Result. Call `WasmEdge_ResultGetMessage` for the error
