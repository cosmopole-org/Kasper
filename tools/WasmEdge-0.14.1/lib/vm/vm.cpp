...
Expect<std::vector<std::pair<ValVariant, ValType>>>
VM::unsafeExecuteWithNotifier(std::string_view Func,
                              Span<const ValVariant> Params,
  ...
  if (ActiveModInst) {
    // Execute function and return values with the module instance.
    return unsafeExecuteFuncWithNotifier(ActiveModInst.get(), Func, Params,
                                         ParamTypes, vmId, isPaused,
                                         notifyInstCount, check, false);
  }
  if (ActiveCompInst) {
...
Expect<std::vector<std::pair<ValVariant, ValType>>>
VM::unsafeConExecuteWithNotifier(std::string_view Func,
                                 Span<const ValVariant> Params,
                                 Span<const ValType> ParamTypes, int vmId,
                                 bool (*isPaused)(int inx),
                                 void (*notifyInstCount)(int inx, uint64_t ic),
...
Expect<std::vector<std::pair<ValVariant, ValType>>>
VM::unsafeExecuteFuncWithNotifier(
  ...
  // Execute function.
  if (auto Res = ExecutorEngine.invokeWithNotifier(
          FuncInst, Params, ParamTypes, vmId, isPaused, notifyInstCount, check, conExec);
      unlikely(!Res)) {
    if (Res.error() != ErrCode::Value::Terminated) {
