...
WASMEDGE_CAPI_EXPORT WasmEdge_Result
WasmEdge_VMExecuteWithNotifier(WasmEdge_VMContext *Cxt, const WasmEdge_String FuncName,
                   const WasmEdge_Value *Params, const uint32_t ParamLen,
  ...
  return wrap(
      [&]() {
        return Cxt->VM.executeWithNotifier(genStrView(FuncName), ParamPair.first,
                               ParamPair.second, vmId, isPaused, notifyInstCount, check);
      },
      [&](auto &&Res) { fillWasmEdge_ValueArr(*Res, Returns, ReturnLen); },
      Cxt);
}
...
WASMEDGE_CAPI_EXPORT WasmEdge_Result
WasmEdge_VMConExecuteWithNotifier(WasmEdge_VMContext *Cxt, const WasmEdge_String FuncName,
                   const WasmEdge_Value *Params, const uint32_t ParamLen,
                   WasmEdge_Value *Returns, const uint32_t ReturnLen, int vmId,
                   bool (*isPaused)(int inx), void (*notifyInstCount)(int inx, uint64_t ic), void (*check)(int inx)) {
  auto ParamPair = genParamPair(Params, ParamLen);
  return wrap(
      [&]() {
        return Cxt->VM.conExecuteWithNotifier(genStrView(FuncName), ParamPair.first,
                               ParamPair.second, vmId, isPaused, notifyInstCount, check);
      },
      [&](auto &&Res) { fillWasmEdge_ValueArr(*Res, Returns, ReturnLen); },
      Cxt);
