...
std::unordered_map<
    int, std::tuple<AST::InstrView::iterator, AST::InstrView::iterator,
...
Expect<void> Executor::continueFunctionKasper(
    Runtime::StackManager &, const Runtime::Instance::FunctionInstance &,
    int vmId, bool (*isPaused)(int inx),
    void (*notifyInstCount)(int inx, uint64_t ic), void (*check)(int inx)) {

  locker.lock();
  Runtime::StackManager sm = *std::get<2>(checkpoint[vmId]);
  locker.unlock();

  auto Res =
      execute(sm, std::get<0>(checkpoint[vmId]), std::get<1>(checkpoint[vmId]),
              vmId, isPaused, notifyInstCount, check);
...
Expect<void> Executor::runFunctionKasper(
    Runtime::StackManager &StackMgr,
    const Runtime::Instance::FunctionInstance &Func,
    Span<const ValVariant> Params, int vmId, bool (*isPaused)(int inx),
    void (*notifyInstCount)(int inx, uint64_t ic), void (*check)(int inx)) {
  // Set start time.
  ...
  checkpoint.insert({vmId, {StartIt, Func.getInstrs().end(), NULL}});
  ...
  if (Res) {
    ...
    Res = execute(StackMgr, StartIt, Func.getInstrs().end(), vmId, isPaused,
...
Expect<void> Executor::execute(Runtime::StackManager &StackMgr,
                               ...
                               const AST::InstrView::iterator End, int vmId,
  ...
  while (PC != PCEnd) {
    if ((isPaused != NULL) && isPaused(vmId)) {
      Runtime::StackManager *st = new Runtime::StackManager();
      *st = StackMgr;
      locker.lock();
      std::get<2>(checkpoint[vmId]) = st;
      std::get<0>(checkpoint[vmId]) = PC;
      locker.unlock();
      break;
    }
    Stat->incInstrCount();
    if (notifyInstCount != NULL) {
      notifyInstCount(vmId, Stat->getInstrCount());
    }
    if (Stat) {
